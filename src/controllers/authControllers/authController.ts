  ...
  try {
    ...
    if (user.activeSessionToken) {
      ...
      const loginTime = user.sessionLoginTime
        ? new Date(user.sessionLoginTime).toLocaleString()
        : "Unknown time";
    ...
    await logUserAction(
      ...
      `Login successful from IP: ${req.ip}, Device: ${deviceInfo}`
    );
...
export const addMember = async (req: Request, res: Response) => {
  const { email, password, role, roles, firstName, lastName, machineIds } =
    req.body;
  ...
  if (Array.isArray(roles)) {
    ...
    for (const roleItem of roles) {
      if (!VALID_ROLES.includes(roleItem)) {
        throw new AppError(
          `Invalid role: ${roleItem}. Must be one of: ${VALID_ROLES.join(
            ", "
          )}`,
          400
        );
...
export const getRoles = async (_req: Request, res: Response) => {
  res.json({
    ...
    data: {
      roles: VALID_ROLES,
      description:
        "Available roles for user assignment. You can assign multiple roles by sending an array.",
    },
  });
};
