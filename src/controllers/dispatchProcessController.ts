...
export const updateDispatchProcess = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (dispatchedQty > 0) {
      ...
      const totalFinishedGoodsNeeded = Math.max(0, newTotalDispatched - qcQuantity);
      ...
      if (existingDispatchProcess?.totalDispatchedQty && existingDispatchProcess.totalDispatchedQty > 0) {
        // Previous total finished goods needed
        const previousTotalFinishedGoodsNeeded = Math.max(0, existingDispatchProcess.totalDispatchedQty - qcQuantity);
      ...
      console.log(`📊 [updateDispatchProcess] Finished Goods Calculation (Cumulative):`, {
        totalDispatchedQty: newTotalDispatched,
        qcQuantity,
        totalFinishedGoodsNeeded,
        previousTotalDispatchedQty: existingDispatchProcess?.totalDispatchedQty || 0,
    ...
    // Prisma: DispatchProcess has operatorName only (strip oprName, machine, machineNo)
