...
async function debugPaperStoreFilteringDetailed() {
  try {
    console.log('🔍 Debugging Paper Store filtering in detail...');
    
    // 1. Log in with a user that has paperstore role
    console.log('\n1. Logging in with user that has paperstore role...');
    const loginResponse = await axios.post(`${BASE_URL}/auth/login`, {
      email: 'allroles@ex.com', // This user should have multiple roles including paperstore
      password: '1234567'
    });
    ...
    // 2. Check if user has paperstore role
    const hasPaperStoreRole = user.roles && user.roles.includes('paperstore');
    console.log(`✅ Has paperstore role: ${hasPaperStoreRole}`);
    
    if (!hasPaperStoreRole) {
      console.log('❌ User does not have paperstore role, cannot test filtering');
      return;
    }
    ...
    // 5. Check if there are any jobs with PaperStore steps
...
debugPaperStoreFilteringDetailed();
